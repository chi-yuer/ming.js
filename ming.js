...
;(async function () {
    if (window.customElements.get('m-select')
    ...
    scriptArray.forEach(function (scriptNode) {
        let src = scriptNode.hasAttribute('src') ? scriptNode.getAttribute('src').toString() : '';
        if ((new RegExp(/ming.js$/)).test(src)) {
            mingScript = scriptNode;
    ...
    const syncLoadDependency = {
        getXmlHttpRequest: function () { //获取XMLHttpRequest对象(提供客户端同http服务器通讯的协议)
            if (window.XMLHttpRequest) // 除了IE外的其它浏览器
                return new XMLHttpRequest();
            else if (window.ActiveXObject) // IE 
                return new ActiveXObject('MsXml2.XmlHttp');
        },
        createStyleElement: function (rootObject, fileUrl) { //导入文件
            if (rootObject != null) {
        ...
        createStyleContent: function (rootObject, styleContent) { //导入文件
            if (rootObject != null) {
                let oStyle = document.createElement('style');
                oStyle.setAttribute('rel', 'stylesheet');
                oStyle.setAttribute('type', 'text/css');
        ...
        createScriptElement: function (rootObject, fileUrl, thisObject) { //导入文件
            if (rootObject != null) {
        ...
        createScriptContent: function (rootObject, scriptContent) { //导入文件
            if (rootObject != null) {
                let oScript = document.createElement('script');
                oScript.type = 'text/javascript';
                ...
                if (oXmlHttp.readyState == 4) { //当执行完成以后(返回了响应)所要执行的        
                    if (oXmlHttp.status == 200 || oXmlHttp.status == 304) { //200有读取对应的url文件,404表示不存在这个文件        
                        ...
                        let info = {
                            ...
                            message: `request error <(${oXmlHttp.status})${oXmlHttp.statusText}>`,
                        };
                        mError.apply(info);
                    }
                }
    ...
    if (heads.length) {
        ...
        syncLoadDependency.loadStyle(document.documentElement, `${url}iconfont/iconfont.css`);
    }
    // 引入fly.js
    ...
    if (body.length) {
        ...
        syncLoadDependency.loadScript(document.documentElement, `${url}fly/fly.min.js`);
    }
    ...
    function mError() {
        ...
        eval(`console.error('%cerror', 'color:#b7282e', '${this.name}:', this.message${paramStr})`);
    }
    ...
    function mWarn() {
        ...
        eval(`console.warn('%cwarn', 'color:#ee7948', '${this.name}:', this.message${paramStr})`);
    }

    // 平滑滚动
    const ScrollTop = (obj, number = 0, time) => {
        if (!time) {
    ...
    function configMingTable(config) {
        ...
        if (param.container.nodeName != 'M-TABLE') { // 容器为非m-table标签
            let info = {
                name: 'm-table',
        ...
        if (!param.columns) { // 尚未给出columns配置
            let info = {
                name: 'm-table',
            ...
            if (!Array.isArray(param.columns)) { // columns为非数组
                let info = {
                    name: 'm-table',
            ...
            if (param.columns.length == 0) { // columns数组为空
                let info = {
                    name: 'm-table',
            ...
            if (Array.isArray(param.columns[0])) { // columns内的元素为数组
        ...
        try {
            ...
            param.columns[0].forEach(function (item) { // 按行解析
        ...
        try {
            param.columns.forEach(function (row, rowIndex) { // 按行解析
                let i = 0;
                let tableHeaderRow = '';
                let fixedTableHeaderRow = '';
                row.forEach(function (column, colIndex) { // 按列解析
                    while (param.renderGrid[rowIndex][i] === false && i < param.renderGrid[rowIndex].length) {
                        i++;
                    }
            ...
            param.renderList.forEach(function (item, index) {
                emptyTr += `<th col-index="c${index}"` +
                    `${index == 0 ? ' class="border-left-column"' : ''}` +
                    `${index == param.renderList.length - 1 ? ' class="border-right-column"' : ''}></th>`;
                if (item.fixed) {
                ...
                if (node.nodeName = 'LI' && node.hasAttribute('action-target')) {
                    ...
                    switch (target) {
                        ...
                        case 'next-page':
                            ...
                            break;
                        case 'last-page':
                            param.container.config.pageNumber = param.container.config.maxPageNumber;
                            param.container.render();
                            break;
                        default:
                    }
    ...
    async function renderMingTable() {
        ...
        if (typeof param.tableData == 'function') {
            param.tableData = await eval('this.config.data()');
        }
        if (!param.renderList || !param.config) { // 验证组件是否已经注册
            let info = {
                name: 'm-table',
        ...
        if (Object.prototype.toString.call(param.tableData) != '[object Object]') { // 验证数据格式是否正确
            let info = {
                name: 'm-table',
        ...
        if (param.tableData.status == 200) {
            ...
            try {
                let thVisableNodes = Array.from(param.container.querySelectorAll('.m-table-main-header tr:first-of-type th'));
            ...
            try {
                ...
                let fixedTableBody = param.container.querySelector('.m-table-fixed-body');
                mainTableBody.innerHTML = '';
                fixedTableBody.innerHTML = '';
                param.tableData.data.forEach(function (row, rowIndex) { // 按行解析
                    ...
                    param.renderList.forEach(function (column, colIndex) { // 按解析列表解析列
                        let columnValue = row[column.field] || '-';
                        if (column.formatter) { // 如果存在自定义数据格式化
                            columnValue = eval(`column.formatter(row, row.${column.field}, rowIndex)`);
                        }
                        ...
                        mainColumn.querySelector('m-inner-cell.inner-cell').dataSet = {row: row, column: columnValue, rowIndex: rowIndex, colIndex: colIndex};
                        mainRow.appendChild(mainColumn);
                        mainRow.dataset = new Object({row: row, column: columnValue, rowIndex: rowIndex, colIndex: colIndex}).toString();
                        if (column.fixed) {
                            let fixedColumn = document.createElement('td');
                            fixedColumn.setAttribute('col-index', 'c' + colIndex);
                            if (rowIndex == 0) fixedColumn.setAttribute('align', column.align);
                            if (rowIndex == 0) fixedColumn.setAttribute('valign', column.valign);
                            fixedColumn.innerHTML = `<m-inner-cell class="inner-cell">${columnValue}</m-inner-cell>`;
                            fixedColumn.querySelector('m-inner-cell.inner-cell').dataSet = {row: row, column: columnValue, rowIndex: rowIndex, colIndex: colIndex};
            ...
            try {
                ...
                pageSelector.push({
                    ...
                    active: false,
                    target: 'first-page',
                    value: pageTypeIcon ? '<i class="m-iconfont ming-icon-first"></i>' : '首页',
                });
                pageSelector.push({
                    disable: firstStatus,
                    active: false,
                    target: 'previous-page',
                    value: pageTypeIcon ? '<i class="m-iconfont ming-icon-previous"></i>' : '上一页',
                });
                if (currentPage - 2 >= 1) {
                    pageSelector.push({
                        disable: false,
                ...
                pageSelector.push({
                    ...
                    active: false,
                    target: 'next-page',
                    value: pageTypeIcon ? '<i class="m-iconfont ming-icon-next"></i>' : '下一页',
                });
                pageSelector.push({
                    disable: lastStatus,
                    active: false,
                    target: 'last-page',
                    value: pageTypeIcon ? '<i class="m-iconfont ming-icon-last"></i>' : '尾页',
                });
                ...
                pageSelector.forEach(function (selector) {
                    ...
                    if (selector.target) slt.setAttribute('action-target', selector.target);
    ...
    function syncFixedTable() {
        ...
        if (!container.querySelector('.m-table-fixed-header') && !container.querySelector('.m-table-fixed-body')) {
            return false;
        }
        let renderList = container.renderList;
        renderList.forEach(function (item, index) {
            let thNodes = Array.from(container.querySelector('.m-table-main-header').querySelectorAll(`th[col-index=c${index}]`));
            let thNode = thNodes ? thNodes[thNodes.length - 1] : null; // 标题node
            let thFirstNode = thNodes ? thNodes[0] : null; // 标题node
            let tdNode = container.querySelector('.m-table-main-body tr[row-index=r0]').querySelector(`td[col-index=c${index}]`);
            if (thFirstNode && thNode && tdNode) {
                if (thFirstNode.style.width && tdNode.style.width) {
                    return false;
                }
                let maxWidth = (tdNode.previousElementSibling == null || tdNode.nextElementSibling == null) ? 35 : 30;
                if (item.width) {
                    ...
                    maxWidth += Math.max(thNode.querySelector('.inner-cell').clientWidth, tdNode.querySelector('.inner-cell').clientWidth);
                }
                thFirstNode.style.width = maxWidth + 'px';
                tdNode.style.width = maxWidth + 'px';
            }
        });
        let fixedVisableHeaderNodes = Array.from(container.querySelectorAll('.m-table-fixed-header tr:first-of-type th'));
        fixedVisableHeaderNodes.forEach(function (fixedVisableHeaderNode) {
            ...
            let referenceNode = container.querySelector(`.m-table-main-header tr[row-index=r-1] th[col-index=${colIndex}]`);
            if (referenceNode) {
                fixedVisableHeaderNode.style.width = referenceNode.style.width;
            }
        });
        let fixedHeaderNodes = Array.from(container.querySelectorAll('.m-table-fixed-header tr:not(:first-of-type) th'));
        fixedHeaderNodes.forEach(function (fixedHeaderNode) {
            ...
            let referenceNode = container.querySelector(`.m-table-main-header tr[row-index=${rowIndex}] th[col-index=${colIndex}]`);
            if (referenceNode) {
                fixedHeaderNode.style.height = referenceNode.clientHeight + 'px';
            }
        });
        let fixedBodyHorizontalNodes = Array.from(container.querySelectorAll('.m-table-fixed-body tr:first-of-type td'));
        fixedBodyHorizontalNodes.forEach(function (fixedBodyHorizontalNode) {
            ...
            let referenceNode = container.querySelector(`.m-table-main-body tr:first-of-type td[col-index=${colIndex}]`);
            if (referenceNode) {
                fixedBodyHorizontalNode.style.width = referenceNode.clientWidth + 'px';
            }
        });
        let fixedBodyVerticalNodes = Array.from(container.querySelectorAll('.m-table-fixed-body td:first-of-type'));
        fixedBodyVerticalNodes.forEach(function (fixedBodyVerticalNode) {
            ...
            let referenceNode = container.querySelector(`.m-table-main-body tr[row-index=${rowIndex}] td`);
            if (referenceNode) {
        ...
        if (scrollDiff > 0 && tableBody.scrollLeft != scrollDiff && !fixedBody.classList.contains('more')) {
        ...
        if (container.scrollHeight > container.clientHeight) {
            let scrollHeight = tableBody.offsetHeight - mainHeader.clientHeight - mainBody.clientHeight;
            mainBody.style.height = container.clientHeight - mainHeader.clientHeight - tableFooter.clientHeight - scrollHeight + 'px';
    ...
    function insertScript(scriptStrList, obj) {
        if (Array.isArray(scriptStrList) && scriptStrList.length) {
            ...
            let content = scriptItem.replace(/<script[^>]*>/g, '').replace('</script>', '');
            scriptEle.setAttribute('type', 'text/javascript');
            if (src && src[0]) {
                scriptEle.setAttribute('src', src[0].replace('src="', '').replace('"', ''));
            }
            if (content) {
                scriptEle.text = content;
            }
            obj.appendChild(scriptEle);
            if (scriptEle.readyState) { // ie中的准备状态
    ...
    class mOption extends HTMLElement {
        ...
        initComponent() {
            ...
            this.parentNode.calcWidthAndPosition(); // 重新计算列表宽度和位置
        ...
        clickEventListener(e) {
            if (this.parentNode.disabled || this.disabled) { // 选项禁用
                return false;
            }
            if (this.selected) { // 选项已选中
                return false;
            }
            this.selected = true;
        }
        ...
        set selected(value) {
            if (value) {
                ...
                Array.from(this.parentNode.querySelectorAll('m-option[selected]')).forEach(function (node) {
                    node.selected = false;
                });
                this.setAttribute('selected', '');
                this.parentNode.shadowRoot.querySelector('.m-select span').innerHTML = this.innerHTML;
            } else {
    ...
    class mSelect extends HTMLElement {
        ...
        initComponent() {
            ...
            let nodeSelected = nodeArray.length > 0 ? nodeArray[nodeArray.length - 1] : this.querySelector(`m-option`);
            ...
            let html = `
                ...
                <style type="text/css">
                    ...
                    :host(m-select) .m-select-option > div::before {
                        content: "";
                        width: 100%;
                        height: 4px;
                    ...
                    :host(m-select) .m-select-option > div {
                        width: 100%;
                        height: fit-content;
                        padding: 8px 0;
                        list-style: none;
            ...
            this.shadowRoot.querySelector('.m-select-option').addEventListener('mouseenter', function (e) {
                if (self.listCloseDelay) {
                    clearTimeout(self.listCloseDelay);
                    self.listCloseDelay = null;
                }
            });
            this.shadowRoot.querySelector('.m-select-option').addEventListener('mouseover', function (e) {
                if (self.listCloseDelay) {
                    clearTimeout(self.listCloseDelay);
                    self.listCloseDelay = null;
                }
        ...
        addHoverState() {
            ...
            this.shadowRoot.querySelector('.m-select i').classList.add('arrow-rotate');
            this.shadowRoot.querySelector('.m-select-option').style.top = this.clientHeight + 3 + 'px';
            ...
            this.shadowRoot.querySelector('.m-select-option').style.height = optionLength * 30 + 8 * 2 + 20 + 'px';
        ...
        removeHoverState() {
            ...
            this.shadowRoot.querySelector('.m-select-option').classList.remove('active');
            this.shadowRoot.querySelector('.m-select i').classList.remove('arrow-rotate');
        ...
        calcWidthAndPosition() {
            ...
            optionArray.forEach(function (node) {
                ...
                self.optionWidth = self.optionWidth > node.clientWidth ? self.optionWidth : node.clientWidth;
                node.style.width = '100%';
            });
            this.shadowRoot.querySelector('.m-select').style.width = self.optionWidth + 32 + 'px';
            this.shadowRoot.querySelector('.m-select-option').style.width = self.optionWidth + 2 + 'px';
        }
    ...
    class mOperation extends HTMLElement {
        ...
        clickEventListener(e) {
            if (this.parentNode.disabled || this.disabled) { // 操作是否禁用
                return false;
            }
            if (this.operation) {
                try {
    ...
    class mOperationList extends HTMLElement {
        ...
        initComponent() {
            ...
            let html = `
                ...
                <style type="text/css">
                    ...
                    :host(m-operation-list) .m-operation-list {
                        ...
                        overflow: hidden;
                        opacity: 0;
                        transition: height .3s ease, opacity .1s linear;
                        position: absolute;
                        top: 36px;
                        left: -8px;
                        z-index: 0;
                    ...
                    :host(m-operation-list) .m-operation-list > div::before {
                        content: "";
                        width: 100%;
                        height: 4px;
                        background-image: url("${url}images/select_arrow.png");
                    ...
                    :host(m-operation-list) .m-operation-list > div {
                        width: 100%;
                        padding: 8px 0;
                        list-style: none;
                        border-radius: 4px;
            ...
            `;
            shadow.innerHTML = html;
            this.calcWidthAndPosition(); // 计算宽度和位置
            this.addEventListener('mouseenter', this.addHoverState);
            this.addEventListener('mouseout', function (e) {
                self.listCloseDelay = setTimeout(function () {
                    self.removeHoverState();
            ...
            this.shadowRoot.querySelector('.m-operation-list').addEventListener('mouseenter', function (e) {
               if (self.listCloseDelay) {
                   clearTimeout(self.listCloseDelay);
                   self.listCloseDelay = null;
               }
        ...
        addHoverState() {
            ...
            this.calcWidthAndPosition();
            this.shadowRoot.querySelector('.m-operation-tip .arrow').classList.add('arrow-rotate');
            let optionLength = Array.from(this.querySelectorAll('m-operation')).length;
            this.shadowRoot.querySelector('.m-operation-list').classList.add('active');
            this.shadowRoot.querySelector('.m-operation-list').style.height = optionLength * 30 + 8 * 2 + 20 + 'px';
            this.shadowRoot.querySelector('.m-operation-list').style.zIndex = 999;
        }

        removeHoverState() {
            this.shadowRoot.querySelector('.m-operation-tip .arrow').classList.remove('arrow-rotate');
            ...
            this.shadowRoot.querySelector('.m-operation-list').classList.remove('active');
        }

        calcWidthAndPosition() { // 计算列表宽度和位置
            let self = this;
            self.optionWidth = 0;
            ...
            let tipWidth = this.shadowRoot.querySelector('.m-operation-tip').clientWidth;
            this.shadowRoot.querySelector('.m-operation-list').style.width = self.optionWidth + 'px';
            this.shadowRoot.querySelector('.m-operation-list').style.minWidth = tipWidth + 'px';
            let left = (tipWidth - self.optionWidth) / 2;
            left = (left > 0 ? 0 : left) - 8;
            this.shadowRoot.querySelector('.m-operation-list').style.left = left + 'px';
        }
        ...
        set disabled(value) {
            if (value) {
                ...
                this.removeAttribute('disabled');
                this.shadowRoot.querySelector('.m-operation').classList.remove('disabled');
            }
        }
    ...
    class mFunction extends HTMLElement {
            ...
            let html = `
                ...
                <div class="m-function${disabled ? ' disabled' : ''}">&nbsp;
                    <div class="m-function-content">
                        <i class="m-iconfont ${iconClass ? iconClass: (icon ? '' : 'icon-ios-help-circle-outl')}">${icon ? icon: ''}</i>
        ...
        mouseEnterListener() {
            ...
            if (this.resetZIndex) { // 清除时间差导致的z-index重置延迟
                clearTimeout(this.resetZIndex);
                this.resetZIndex = null;
            }
            // 清除容器内所有m-function的z-index延迟并手动重置
            Array.from(this.parentNode.querySelectorAll('m-function')).forEach(function (node) {
                if (node.resetZIndex) {
                    clearTimeout(node.resetZIndex);
                    node.resetZIndex = null;
                }
                node.shadowRoot.querySelector('.m-function-content').style.zIndex = 'auto';
            });
            // 设置层级并设置展开的宽度
            this.shadowRoot.querySelector('.m-function-content').style.zIndex = '999';
            this.shadowRoot.querySelector('.m-function-tip').style.width = this.tipWidth + 'px';
        }

        mouseLeaveListener() {
            let self = this;
            this.shadowRoot.querySelector('.m-function-tip').style.width = 0; // 将提示内容隐藏
            this.resetZIndex = setTimeout(function () { // 设置 z-index 的延时重置
                self.shadowRoot.querySelector('.m-function-content').style.zIndex = 'auto';
            }, 300);
        }
        ...
        set disabled(value) {
            if (value) {
                ...
                this.removeAttribute('disabled');
                this.shadowRoot.querySelector('.m-function').classList.remove('disabled');
            }
        }
    ...
    class mIconFunction extends HTMLElement {
        ...
        initComponent() {
            ...
            this.shadowRoot.querySelector('.m-function-tip-up').style.width = tipWidth + 16 + 'px';
            this.shadowRoot.querySelector('.m-function-tip-down').style.width = tipWidth + 16 + 'px';
            this.shadowRoot.querySelector('.m-function-tip-up').style.left = -tipWidth/2 + 8 + 'px';
            this.shadowRoot.querySelector('.m-function-tip-down').style.left = -tipWidth/2 + 8 + 'px';
            this.shadowRoot.querySelector('.visible-tip').remove();
            this.addEventListener('mouseenter', this.mouseEnterListener);
            this.addEventListener('click', function (e) {
                if (self.disabled) { // 禁用判断
        ...
        mouseEnterListener() {
            ...
            if (top && top > 80) {
                this.shadowRoot.querySelector('.m-function').classList.remove('m-function-down');
                this.shadowRoot.querySelector('.m-function').classList.add('m-function-up');
            } else {
                this.shadowRoot.querySelector('.m-function').classList.remove('m-function-up');
                this.shadowRoot.querySelector('.m-function').classList.add('m-function-down');
            }
        }
    ...
    class mMenuItem extends HTMLElement {
        ...
        clickEventListener(e) {
            if (this.classList.contains('active')) { // 菜单已经激活
                return false;
            }
            if (this.parentNode.disabled || this.disabled) { // 菜单组或菜单已禁用
                return false;
            }
            let self = this;
            let menuItemList = Array.from(this.parentNode.querySelectorAll('m-menu-item'));
            ...
            if (this.menuRoute) {
                if (this.parentNode.parentNode.menuTarget) {
                    if (layer) {
                        self.parentNode.parentNode.loading = layer.load(2, {shade: [0.3, '#000']});
                    }
                    if (this.parentNode.parentNode.menuTarget.nodeName == 'IFRAME') {
                        this.parentNode.parentNode.menuTarget.setAttribute('src', this.menuRoute);
                        if (!this.parentNode.parentNode.menuTarget.attachEvent) {
                            this.parentNode.parentNode.menuTarget.onload = this.parentNode.parentNode.clearLoading.apply(this.parentNode.parentNode,[]);
                        }
                    } else {
                        fly.get(this.menuRoute)
                            .then(function (response) {
                                if (response.status == 200) {
                                    let originStr = response.data.toString().replace(/\r|\n|\\s/g, '');
                                    let linkList = Array.from(originStr.match(new RegExp(/<link((?!<>).)*>/g)) || []);
                                    let styleList = Array.from(originStr.match(new RegExp(/<style((?!<\/style>).)*<\/style>/g)) || []);
                                    let scriptList = Array.from(originStr.match(new RegExp(/<script((?!<\/script>).)*<\/script>/g)) || []);
                                    let bodyHtml = originStr.match(new RegExp(/<body>(.)*<\/body>/))[0];
                                    linkList.forEach(function (link) {
                                        ...
                                        if (href && href[0]) {
                                            linkEle.setAttribute('href', href[0].replace('href="', '').replace('"', ''));
                                        }
                                        self.parentNode.parentNode.menuTarget.appendChild(linkEle);
                                    });
                                    styleList.forEach(function (style) {
                                        let styleEle = document.createElement('style');
                                        let content = style.replace(/<style[^>]*>/g, '').replace('</style>', '');
                                    ...
                                    bodyHtml = bodyHtml.replace(/<script((?!<\/script>).)*<\/script>/g, '');
                                    self.parentNode.parentNode.menuTarget.innerHTML += bodyHtml;
                                    insertScript(scriptList, self.parentNode.parentNode.menuTarget);
                                }
                            })
    ...
    class mMenuGroup extends HTMLElement {
        ...
        initComponent() { // 组件初始化
            ...
            let icon = {
                ...
                type: new RegExp(/^(\&\#)(.)*(;)$/).test(this.getAttribute('icon')) ? 'unicode' : 'class',
            };
            ...
            let html = `
                ...
                <div class="m-menu-group ${disabled ? 'disabled' : ''}">
                    <div class="group-icon">
                        <i class="m-iconfont ${icon.type == 'class' ? icon.content : ''}">${icon.type == 'unicode' ? icon.content : ''}</i>
                    </div>
        ...
        clickEventListener(e) {
            ...
            if (this.disabled) { // 菜单已禁用
                return false;
            }
            let self = this;
            // 清除所有二级菜单的激活样式
            let menuItemList = Array.from(this.parentElement.querySelectorAll('m-menu-item'));
            menuItemList.forEach(function (menuItem) {
                if (menuItem.classList.contains('active')) {
                    menuItem.classList.remove('active');
                }
            ...
            // 清除其余菜单组的激活样式
            let menuGroupList = Array.from(this.parentElement.querySelectorAll('m-menu-group'));
                    ...
                    if (this.parentNode.menuTarget) {
                        ...
                        if (this.parentNode.menuTarget.nodeName == 'IFRAME') {
                            ...
                            if (!this.parentNode.menuTarget.attachEvent) {
                                this.parentNode.menuTarget.onload = this.parentNode.clearLoading.apply(this.parentNode,[]);
                            }
                        } else {
                            fly.get(this.menuRoute)
                                .then(function (response) {
    ...
    class mTable extends HTMLElement {
        ...
        initComponent() { // 初始化组件
            ...
            this.shadowRoot.querySelector('.table-main').addEventListener('mousewheel', function (e) {
                e.preventDefault();
            ...
            this.shadowRoot.querySelector('.fixed-table').addEventListener('mousewheel', function (e) {
                e.preventDefault();
                let top = self.shadowRoot.querySelector('.table-main').scrollTop;
                let scrollStep = 50;
                let result = 0;
            ...
            this.shadowRoot.querySelector('.fixed-table').addEventListener('scroll', function (e) {
                e.preventDefault();
                self.shadowRoot.querySelector('.table-main').scrollTop = self.shadowRoot.querySelector('.fixed-table').scrollTop;
                syncFixedTable.call(self);
            });
        }
